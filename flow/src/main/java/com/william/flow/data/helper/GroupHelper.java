...
import com.william.flow.model.db.GroupMember_Table;
import com.william.flow.model.db.Group_Table;
...
public class GroupHelper {
    ...
    public static Group findFormNet(String id) {
        ...
        try {
            ...
            if (card != null) {
                // 数据库的存储并通知
                Flow.getGroupCenter().dispatch(card);
                ...
                .enqueue(new Callback<RspModel<GroupCard>>() {
                    ...
                    public void onResponse(Call<RspModel<GroupCard>> call, Response<RspModel<GroupCard>> response) {
                        ...
                        if (rspModel.success()) {
                            ...
                            // 唤起进行保存的操作
                            Flow.getGroupCenter().dispatch(groupCard);
                            // 返回数据
            ...
            public void onResponse(Call<RspModel<List<GroupCard>>> call, Response<RspModel<List<GroupCard>>> response) {
                ...
                if (rspModel.success()) {
                    ...
                    if (groupCards != null && groupCards.size() > 0) {
                        // 进行调度显示
                        Flow.getGroupCenter().dispatch(groupCards.toArray(new GroupCard[0]));
                    }
                } else {
            ...
            public void onResponse(Call<RspModel<List<GroupMemberCard>>> call, Response<RspModel<List<GroupMemberCard>>> response) {
                ...
                if (rspModel.success()) {
                    ...
                    if (memberCards != null && memberCards.size() > 0) {
                        // 进行调度显示
                        Flow.getGroupCenter().dispatch(memberCards.toArray(new GroupMemberCard[0]));
                    }
                } else {
                    Flow.decodeRspCode(rspModel, null);
                }
    ...
    //网络请求加入一个群
    public static void applyJoinGroup(ApplyModel applyModel, final DataSource.Callback<ApplyCard> callback) {
        RemoteService service = Network.remote();
        Call<RspModel<ApplyCard>> call = service.joinGroup(applyModel);
        call.enqueue(new Callback<RspModel<ApplyCard>>() {
            @Override
            public void onResponse(Call<RspModel<ApplyCard>> call, Response<RspModel<ApplyCard>> response) {
                RspModel<ApplyCard> rspModel = response.body();
                if (rspModel.success()) {
                    ApplyCard card=rspModel.getResult();
            ...
            public void onFailure(Call<RspModel<ApplyCard>> call, Throwable t) {
             callback.onDataNotAvailable(R.string.data_rsp_error_join_group);
