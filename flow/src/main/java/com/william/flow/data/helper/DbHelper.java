...
public class DbHelper {
    ...
    private final <Model extends BaseModel> void notifySave(final Class<Model> tClass,
        ...
        else if(Apply.class.equals(tClass))
        {
            updateSession((Apply[])models);
        }
    }
...
private void updateSession(final Apply... applies)
{
    final Set<Session.Identify> identifies=new HashSet<>();
    int lenth=0;
    for (Apply apply : applies) {
        if(apply.getApplicantId().equalsIgnoreCase(Account.getUserId())&&apply.getConfirmType()!=Apply.TYPE_CONFIRMED)
        {
            continue;
        }
        else
    ...
    if(lenth==0)
        return;
    final int sessionLenth=lenth;
    DatabaseDefinition definition=FlowManager.getDatabase(AppDatabase.class);
    definition.beginTransactionAsync(new ITransaction() {
        @Override
        public void execute(DatabaseWrapper databaseWrapper) {
            ModelAdapter<Session> adapter =FlowManager.getModelAdapter(Session.class);
            Session[] sessions=new Session[sessionLenth];
            int index=0;
            for (Apply apply : applies) {
                Session session=SessionHelper.findFromLocal(apply.getId());
                if(session==null)
                {
                    session=new Session(apply);
                }
                session.refreshWithApply(apply);
                adapter.save(session);
                sessions[index++]=session;
            }
            // 调用直接进行一次通知分发
