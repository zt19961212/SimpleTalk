...
import com.william.flow.data.helper.MessageHelper;
...
public class Session extends BaseDbModel<Session> {
    ...
    @Column
    private Date modifyTime; // 最后更改时间

    @ForeignKey(tableClass = Message.class)
    private Message message; // 对应的消息，外键为Message的Id
@ForeignKey(tableClass = Apply.class)
    private Apply apply;

    public Session(Apply apply) {
        if (apply.getType() == Apply.TYPE_ADD_USER) {
            receiverType = Apply.TYPE_ADD_USER;
            title = "好友通知";
        } else {
            receiverType = Apply.TYPE_ADD_GROUP;
            title = "群通知";
        }
        id = apply.getId();
        content = apply.getContent();
        modifyTime = apply.getCreateTime();
        this.apply = apply;
    }
    ...
    public Session(Message message) {
        ...
        this.modifyTime = message.getCreateTime();
    }
    ...
    public boolean equals(Object o) {
        ...
        return receiverType == session.receiverType
                ...
                && Objects.equals(modifyTime, session.modifyTime)
                && Objects.equals(message, session.message)
                && Objects.equals(apply, session.apply);
    }
    ...
    public boolean isUiContentSame(Session oldT) {
        return this.content.equals(oldT.content)
                && Objects.equals(this.modifyTime, oldT.modifyTime);
    }
    ...
    public void refreshWithApply(Apply apply) {
this.content=apply.getContent();
        this.apply=apply;
        this.modifyTime=apply.getUpdateTime();
